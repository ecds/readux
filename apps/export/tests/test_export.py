...
from apps.export.export import (
    IiifManifestExport,
    JekyllSiteExport,
    GithubExportException,
    ExportException,
)
...
class ManifestExportTests(TestCase):
    ...
    def setUp(self):
        ...
        self.sa_app = SocialAppFactory.create(provider="github", name="GitHub")
        ...
        self.sa_token = SocialTokenFactory.create(
            app_id=self.sa_app.pk, account_id=self.sa_acct.pk
        )
    ...
    def test_zip_creation(self):
        ...
        ocr_annotation_list_id = manifest["sequences"][0]["canvases"][0][
            "otherContent"
        ][0]["@id"]
        ocr_annotation_list_path = os.path.join(
            tmpdir, re.sub(r"\W", "_", ocr_annotation_list_id) + ".json"
        )
        ...
        comment_annotation_list_id = [
            x["@id"]
            for x in other_content
            if x["@type"] == "sc:AnnotationList" and "OCR" not in x["label"]
        ][0]

        comment_annotation_list_path = os.path.join(
            tmpdir, re.sub(r"\W", "_", comment_annotation_list_id) + ".json"
        )
    ...
    def test_jekyll_site_export(self):
        user_anno = UserAnnotation.objects.get(
            pk="18f24705-398a-401d-a106-acfca6e72070"
        )
        ...
        assert os.path.exists(
            os.path.join(jekyll_path, "overlays", "annotations", "6.json")
        )
    ...
    def test_jekyll_export_error(self):
        export = JekyllSiteExport(
            self.volume, "v2", owners=[self.user.id], deep_zoom="exclude"
        )
    ...
    def test_jekyll_export_to_github_repo_name_has_spaces(self):
        httpretty.register_uri(
            httpretty.GET, "https://zaphod.github.io/has-spaces/", status=200
        )
        httpretty.register_uri(
            httpretty.GET,
            "https://api.github.com/users/{u}/repos?per_page=3".format(
                u=self.jse.github_username
            ),
            body='[{"name":"marx"}]',
            content_type="text/json",
        )
        httpretty.register_uri(httpretty.POST, "https://api.github.com/user/repos")
        kwargs = {"pid": self.volume.pid, "version": "v2"}
        url = reverse("JekyllExport", kwargs=kwargs)
        kwargs["deep_zoom"] = "exclude"
        kwargs["mode"] = "github"
        ...
        assert "https://github.com/zaphod/has-spaces" in response.content.decode(
            "utf-8"
        )
    ...
    def test_jekyll_export_to_github_repo_name_not_given(self):
        httpretty.register_uri(
            httpretty.GET,
            f"https://zaphod.github.io/{slugify(self.volume.label, lowercase=False, max_length=50)}/",
            status=200,
        )
        httpretty.register_uri(
            httpretty.GET,
        ...
        assert response.status_code == 200
        assert (
            f"https://github.com/zaphod/{slugify(self.volume.label, lowercase=False, max_length=50)}"
    ...
    def test_github_auth_repo_given_name(self):
        ...
        assert (
            auth_repo
            == f"https://{self.jse.github_username}:{self.sa_token.token}@github.com/{self.jse.github_username}/{self.jse.github_repo}.git"
        )

    def test_github_auth_repo_given_url(self):
        auth_repo = self.jse.github_auth_repo(
            repo_url=f"https://github.com/{self.jse.github_username}/{self.jse.github_repo}"
        )
        assert (
            auth_repo
            == f"https://{self.jse.github_username}:{self.sa_token.token}@github.com/{self.jse.github_username}/{self.jse.github_repo}.git"
        )  # .format(t=self.sa_token.token, r=)
    ...
    def test_create_website_gitrepo_when_repo_already_exists(self):
        ...
        with self.assertRaisesMessage(
            GithubExportException,
            "GitHub repo {r} already exists.".format(r=self.jse.github_repo),
        ):
    ...
    def test_website_github_repo(self):
        httpretty.register_uri(
            httpretty.GET,
            "https://{t}:x-oauth-basic@github.com/{u}/{r}.git/".format(
                t=self.jse.github_token,
                u=self.jse.github_username,
                r=self.jse.github_repo,
            ),
            body="",
            status=200,
        )
        ...
        httpretty.register_uri(
            httpretty.GET,
            f"https://{self.jse.github_username}.github.io/{self.jse.github_repo}/",
            status=200,
        )
        resp_body = '[{"name":"foo"}]'
        httpretty.register_uri(
        ...
        assert website == (
            "https://github.com/{u}/{r}".format(
                u=self.jse.github_username, r=self.jse.github_repo
            ),
            "https://{u}.github.io/{r}/".format(
                u=self.jse.github_username, r=self.jse.github_repo
            ),
        )
    ...
    def test_update_githubrepo(self):
        ...
        pull_response_body = '{"html_url":"https://github.com/%s/%s/pull/2"}' % (
            self.jse.github_username,
            self.jse.github_repo,
        )
        httpretty.register_uri(
            httpretty.POST,
        ...
        assert new_pull == "https://github.com/{u}/{r}/pull/2".format(
            u=self.jse.github_username, r=self.jse.github_repo
        )
    ...
    def test_github_export_update(self):
        ...
        assert gh_export == [
            ...
            "https://github.com/{u}/{r}/pull/2".format(
                u=self.jse.github_username, r=self.jse.github_repo
            ),
        ]
    ...
    def test_jekyll_export_to_github_site_timeout(self):
        httpretty.register_uri(
            httpretty.GET, "https://zaphod.github.io/not-found/", status=404
        )
        httpretty.register_uri(
            httpretty.GET,
            "https://api.github.com/users/{u}/repos?per_page=3".format(
        ...
        self.assertEqual(
            ...
            "Your Readux site export is taking longer than expected",
        )
