...
def activate_fake_canvas_info(canvas):
    ...
    httpretty.register_uri(
        httpretty.GET, f"{canvas.resource_id}/info.json", body=iiif_image_info
    )
...
def fetch_positional_ocr(canvas):
    ...
    if "archivelab" in canvas.manifest.image_server.server_base:
        ...
        if environ["DJANGO_ENV"] == "test":
            fake_ocr = open(
                path.join(settings.APPS_DIR, "iiif/canvases/fixtures/ocr_words.json"),
                encoding="utf-8",
            )
            words = fake_ocr.read()
            print(words)
    ...
    if "images.readux.ecds.emory" in canvas.manifest.image_server.server_base:
        ...
        if environ["DJANGO_ENV"] == "test":
            fake_tsv = open(
                path.join(settings.APPS_DIR, "iiif/canvases/fixtures/sample.tsv"),
                encoding="utf-8",
            )
            tsv = fake_tsv.read()
            url = f"https://raw.githubusercontent.com/ecds/ocr-bucket/master/{canvas.manifest.pid}/boo.tsv"
            httpretty.enable()
            httpretty.register_uri(httpretty.GET, url, body=tsv)

        if canvas.ocr_file_path is None:
            pid = (
                canvas.pid.split("_")[-1]
                .replace(".jp2", "")
                .replace(".jpg", "")
                .replace(".tif", "")
            )
            return fetch_url(
                f"https://raw.githubusercontent.com/ecds/ocr-bucket/master/{canvas.manifest.pid}/{pid}.tsv",
                data_format="text",
            )

    url = (
        settings.DATASTREAM_PREFIX
        + canvas.pid.replace("fedora:", "")
        + settings.DATASTREAM_SUFFIX
    )
        ...
        if canvas.image_server.storage_service == "sftp":
            try:
                ...
                sftp = pysftp.Connection(
                    **canvas.image_server.sftp_connection, cnopts=connection_options
                )
                ...
                LOGGER.error(
                    f"Failed to get OCR files from SFTP server with exception: {error}"
                )
                return None
