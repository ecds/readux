...
#            subprocess.check_call(import_command, cwd=tmpdir)
    ...
    # TODO: IIIF export in-place and run the jekyll script        
...
#        jekyll_dir = self.get_jekyll_site_dir(export_dir)
        jekyll_dir = self.edition_dir(export_dir) # TODO make sure this works
...
#        jekyll_dir = self.edition_dir(export_dir) # TODO make sure this works
        ...
        # TODO change to user's settings
...
# from readux/books/consumers.py in Readux 1.
    ...
    #     notify_msg('Export started')
        ...
        if not self.user == None:
            ...
            try:
                # TODO Refactor to not connect twice but use self.github_user instead
    ...
    #     # and add the repo name to the parameters
        if self.export_mode in ['github', 'github_update']:
            try:
                # TODO remove this -- done already above
                github.GithubApi.github_account(self.user)
            except github.GithubAccountNotFound:
                logger.error('TOOO account not found message')
                return

            # check that oauth token has sufficient permission
            # to do needed export steps
            gh = github.GithubApi.connect_as_user(self.user)
            # Readux 1 tested for public_repo, but our default configuration
            # requests repo instead.
            if 'repo' not in gh.oauth_scopes():
