...
class Serializer(JSONSerializer):
    ...
    def get_dump_object(self, obj):
        if (self.version == "v2") or (self.version is None):
            data = {
                "@id": "{h}/iiif/v2/{p}/manifest".format(
                    h=settings.HOSTNAME, p=obj.pid
                ),
                "@type": "sc:Manifest",
                "label": obj.label,
                ...
                "within": [
                    {
                        "@id": "{h}/iiif/v2/{p}/collection".format(
                            h=settings.HOSTNAME, p=collection.pid
                        ),
                        "@type": "sc:Collection",
...
def deserializer():
    ...
    raise SerializerDoesNotExist(
        "all_volumes_manifest is a serialization-only serializer"
    )
