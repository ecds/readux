...
class SerializerTests(TestCase):
    ...
    def test_ocr_annotation(self):
        ...
        assert target['selector']['value'] == f'xywh=pixel:{ocr_anno.x},{ocr_anno.y},{ocr_anno.w},{ocr_anno.h}'
    ...
    def test_fragment_annotation(self):
        ...
        assert body['creator'] == { 'id': frag_anno.owner.username, 'name': frag_anno.owner.name }
        assert target['source'] == canvas.resource_id
        assert target['selector']['value'] == f'xywh=pixel:{frag_anno.x},{frag_anno.y},{frag_anno.w},{frag_anno.h}'
        assert target['selector']['type'] == AnnotationSelector('FR').name
        assert len(serialized_frag_anno['body']) == 1

    def test_svg_annotation(self):
        canvas = CanvasFactory.create(manifest=ManifestFactory.create())
        svg_anno = UserAnnotationFactory.create(canvas=canvas, svg='<svg><path></path></svg>', primary_selector=AnnotationSelector('SV'))
        ...
        assert body['format'] == 'text/plain'
        assert body['creator'] == { 'id': svg_anno.owner.username, 'name': svg_anno.owner.name }
    ...
    def test_text_annotation(self):
        ...
        assert body['format'] == 'text/html'
        assert body['creator'] == { 'id': text_anno.owner.username, 'name': text_anno.owner.name }
        assert text_target['source'] == canvas.resource_id
        assert text_target['selector']['startSelector']['value'] == f"//*[@id='{text_anno.start_selector.pk}']"
        assert text_target['selector']['endSelector']['value'] == f"//*[@id='{text_anno.end_selector.pk}']"
        assert text_target['selector']['startSelector']['refinedBy']['start'] == text_anno.start_offset
        assert text_target['selector']['endSelector']['refinedBy']['end'] == text_anno.end_offset
...
class DeserializerTests(TestCase):
    ...
    def test_web_annotation_comment_range_deserialization(self):
        ...
        assert annotation.start_offset == web_annotation['target']['selector']['startSelector']['refinedBy']['start']
        assert annotation.end_selector == end
        assert annotation.end_offset == web_annotation['target']['selector']['endSelector']['refinedBy']['end']
