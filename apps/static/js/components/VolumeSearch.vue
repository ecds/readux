<template>
  <div class="rx-volume-search">
    ...
    <div class="uk-text-small uk-margin-small-top uk-margin-small-left">
    ...
    <div v-if="hasResults">
      ...
      <ul
        id="rx-search-panel"
        class="uk-switcher uk-panel-scrollable uk-resize-vertical"
        uk-height-viewport="offset-bottom: 100"
      >
        <li>
          <div v-if="inText === 0" class="rx-padding-extra-small uk-text-small">
            No matches in text.
          </div>
          <div
            v-else
            v-for="(match, index) in textData"
            :key="`text-${index}`"
        ...
        <li>
          <div v-if="inAnnotations === 0" class="rx-padding-extra-small uk-text-small">
            No matches in annotations.
          </div>
          <div
            v-else
            v-for="(match, index) in annotationData"
            :key="`anno-${index}`"
            class="rx-padding-extra-small"
          >
            <div class="uk-text-small">
    ...
    <div v-else>
      <div v-if="emptyMessage" class="uk-alert uk-margin-small-top uk-margin-remove-bottom">
        {{ emptyMessage }}
      </div>
    </div>
  </div>
  ...
  methods: {
    async getSearchResults() {
      try {
        ...
        if (this.searchResults.matches_in_annotations) {
        ...
        if (!this.hasResults) {
          this.emptyMessage = "No matches in either annotations or text.";
        } else {
          this.emptyMessage = "";
        }
      } catch (error) {
        console.error("Error fetching volume search data:", error);
