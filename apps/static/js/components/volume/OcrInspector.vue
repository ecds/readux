...
export default {
  ...
  computed: {
    hasText() { return !!(this.pagetext && this.pagetext.trim().length); },
    copyDisabled() { return this.isLoading || !this.hasText; },
    overlayDisabled() { return this.isLoading || !this.hasOcrAvailable || !this.hasText; }
  },
  methods: {
    async copyText() {
      try {
        await navigator.clipboard.writeText(this.pagetext);
        if (window.UIkit?.notification) UIkit.notification({ message: "Copied!", status: "success", timeout: 1200 });
      } catch (err) { console.error("Copy failed:", err); }
    },
    onOverlayToggle() { this.applyOverlay(this.overlayChecked); },
    getOcrNodes() { return document.querySelectorAll("div.openseadragon-canvas div span"); },
  ...
  mounted() {
    this._onCanvasSwitch = (event) => { this.handleCanvasSwitch(event?.detail || {}); };
