...
class BasePage(DigitalObject):
    '''Basic Page object with datastreams for the emory-control:ScannedPage-1.0
    content model.'''
    # NOTE: this object definition is separate from the actual Page object
    # we want to use, which also extends Image, because eulfedora syncrepo
    # only knows how to create content models for DigitalObject classes with
    # only one content model
    PAGE_CONTENT_MODEL = 'info:fedora/emory-control:ScannedPage-1.0'
    CONTENT_MODELS = [PAGE_CONTENT_MODEL]
    ...
    # NOTE: duplicating image datastream declaration here because it should be
    # included in the ScannedPage-1.0 content model
    image = FileDatastream("source-image", "Page image", defaults={
            'mimetype': 'image/tiff',
        })
    ':class:`~eulfedora.models.FileDatastream` with image content'


class Page(BasePage, Image):

    CONTENT_MODELS = [BasePage.PAGE_CONTENT_MODEL, Image.IMAGE_CONTENT_MODEL]
